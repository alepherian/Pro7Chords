...
class FileManagerService: ObservableObject {
    ...
    private func handleProtobufProPresenterFile(data: Data) async throws -> String {
        ...
        do {
            ...
            for (cueIndex, cue) in presentation.cues.enumerated() {
                print("Processing cue \(cueIndex + 1)/\(presentation.cues.count): '\(cue.name)'")
                    ...
                    if action.type == .presentationSlide {
                        ...
                        if case .slide(let slideData) = action.actionTypeData {
                            if case .presentation(let presentationSlide) = slideData.slide {
                                ...
                                // Always add a slide entry to maintain order, even if empty
                                let trimmedText = slideText.trimmingCharacters(in: .whitespacesAndNewlines)
                                if !trimmedText.isEmpty {
                                    allLyrics.append(trimmedText)
                                    print("    📝 Added slide \(slideCount) text (\(trimmedText.count) characters)")
                                } else {
                                    // Add empty placeholder to maintain slide order
                                    allLyrics.append("")
                                    print("    ⚠️ Slide \(slideCount) had no text content - added placeholder")
                                }
                            } else {
                                print("    Slide is not a presentation slide (might be prop slide)")
                            }
            ...
            print("\n📊 Summary: Found \(slideCount) slides total with \(allLyrics.filter { !$0.isEmpty }.count) containing text")
            
            if allLyrics.filter({ !$0.isEmpty }).isEmpty {
                throw FileManagerError.invalidProPresenterFormat("No text content found in any slides")
            }
            
            // Try to sort slides in logical order based on content
            let sortedLyrics = sortSlidesInLogicalOrder(allLyrics)
            
            // Combine all slides with clear separators
            let combinedLyrics = sortedLyrics.joined(separator: "\n\n--- Next Slide ---\n\n")
            print("✅ Successfully extracted \(combinedLyrics.count) characters from \(sortedLyrics.count) slides (sorted in logical order)")
    ...
    // MARK: - Slide Sorting
    private func sortSlidesInLogicalOrder(_ slides: [String]) -> [String] {
        // Create tuples of (slide content, original index)
        let indexedSlides = slides.enumerated().map { (index: $0, content: $1) }
        
        // Filter out empty slides and sort by logical order
        let nonEmptySlides = indexedSlides.filter { !$0.content.isEmpty }
        
        let sortedSlides = nonEmptySlides.sorted { slide1, slide2 in
            let order1 = getSongSectionOrder(slide1.content)
            let order2 = getSongSectionOrder(slide2.content)
            
            if order1.section != order2.section {
                return order1.section < order2.section
            } else {
                return order1.slideNumber < order2.slideNumber
            }
        }
        
        print("🎵 Sorted \(sortedSlides.count) slides in logical song order")
        return sortedSlides.map { $0.content }
    }
    ...
    private func getSongSectionOrder(_ slideContent: String) -> (section: Int, slideNumber: Int) {
        let content = slideContent.lowercased()
        
        // Extract section and slide number from content
        var section = 999 // Default to end
        var slideNumber = 1
        
        // First check for explicit section labels (like test files)
        if content.contains("intro") {
            section = 0
        } else if content.contains("verse 1") {
            section = 1
        } else if content.contains("verse 2") {
            section = 3
        } else if content.contains("verse 3") {
            section = 5
        } else if content.contains("chorus") {
            section = 2
        } else if content.contains("bridge 1") {
            section = 6
        } else if content.contains("bridge 2") {
            section = 7
        } else if content.contains("bridge 3") {
            section = 8
        } else if content.contains("tag") {
            section = 9
        } else if content.contains("outro") || content.contains("end") {
            section = 10
        } else if content.contains("blank") {
            section = 999
        } else {
            // For real songs without labels, try to detect patterns
            section = detectSongSectionFromLyrics(content)
        }
        
        // Extract slide number (look for "slide 1", "slide 2", etc.)
        if let slideMatch = content.range(of: #"slide (\d+)"#, options: .regularExpression) {
            let slideNumberStr = String(content[slideMatch]).replacingOccurrences(of: "slide ", with: "")
            slideNumber = Int(slideNumberStr) ?? 1
        }
        
        return (section: section, slideNumber: slideNumber)
    }
    
    private func detectSongSectionFromLyrics(_ content: String) -> Int {
        // Common verse indicators
        if content.contains("lord of all creation") || 
           content.contains("early in the morning") ||
           content.contains("precious lord reveal") {
            return 1 // Verse
        }
        
        // Common chorus indicators  
        if content.contains("god of wonders") ||
           content.contains("hallelujah to the lord") ||
           content.contains("you are holy") ||
           content.contains("you turn graves into gardens") ||
           content.contains("there's nothing better than you") {
            return 2 // Chorus
        }
        
        // Bridge indicators
        if content.contains("when i stumble") ||
           content.contains("father hold me") ||
           content.contains("i'm not afraid to show") ||
           content.contains("then you came along") {
            return 6 // Bridge
        }
        
        // Tag/ending indicators
        if content.contains("glory to the lord") ||
           content.contains("lord of heaven and earth") ||
           content.contains("oh there's nothing") {
            return 9 // Tag
        }
        
        // If we can't identify, use a neutral position
        return 5
    }
}
