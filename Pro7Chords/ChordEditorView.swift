...
struct ChordEditorView: View {
    ...
    private func setupDefaultContent() {
        ...
        lyrics = defaultText
        AppLogger.info("Set up default content")
    }
    ...
    private func insertChordAtCursor() {
        ...
        // Always insert in ChordPro format
        lyrics = String(beforeCursor) + "[\(selectedChord)]" + String(afterCursor)
        cursorPosition = insertPosition + selectedChord.count + 2
        
        AppLogger.chordProcessing("Inserted chord \(selectedChord) at position \(insertPosition)")
    }
    ...
    private func insertSlideTemplate(_ templateName: String) {
        ...
        AppLogger.info("Inserted template: \(templateName)")
    }
    ...
    private func insertSlideSeparator() {
        ...
        AppLogger.info("Inserted slide separator")
    }
    ...
    private func cleanupUndoStack() {
        ...
        if totalMemory > Self.maxUndoStackMemorySize {
            ...
            if removeCount > 0 {
                ...
                AppLogger.info("Cleaned up undo stack: removed \(removeCount) entries")
            }
        }
    }
    ...
    private func undo() {
        ...
        AppLogger.info("Performed undo operation")
    }
    ...
    private func redo() {
        ...
        AppLogger.info("Performed redo operation")
    }
    
    private func detectKeyFromLyrics() {
        detectedKey = transposer.detectKey(from: lyrics)
        if let key = detectedKey {
            AppLogger.chordProcessing("Detected key: \(key)")
        }
    }
    ...
    private func transposeAllChords(by steps: Int) {
        ...
        AppLogger.chordProcessing("Transposed all chords by \(steps) steps")
    }
    ...
    private func showFileInfo() {
        ...
        Task {
            do {
                ...
                AppLogger.fileOperation("Analyzed ProPresenter file: \(url.lastPathComponent)")
            } catch {
                AppLogger.warning("ProPresenter analysis failed", error: error, category: .fileOperations)
    ...
    private func handleFileLoad(from url: URL) async {
        do {
            ...
            AppLogger.fileOperation("Loaded file: \(url.lastPathComponent)")
        ...
        if savePanel.runModal() == .OK, let saveURL = savePanel.url {
            Task {
                do {
                    ...
                    AppLogger.fileOperation("Saved file: \(saveURL.lastPathComponent)")
                } catch {
    ...
    private func extractChordsFromLyrics() -> [String: String] {
        ...
        for (index, slideText) in slides.enumerated() {
            ...
            if cleanText.contains("[") && cleanText.contains("]") {
                // Store the entire slide text with chords in ChordPro format
